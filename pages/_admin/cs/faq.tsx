...
const AdminFaq: NextPage = () => {
  ...
  const faqList: Notice[] = data?.getNotices?.list ?? [];
  ...
  const createHandler = async () => {
    try {
      ...
      await sweetTopSmallSuccessAlert(t('FAQ created'), 900);
    } catch (err: any) {
  ...
  const editHandler = async () => {
    try {
      ...
      setOpenEdit(false);
      await refetch({ input: inquiry });
      await sweetTopSmallSuccessAlert(t('FAQ updated'), 900);
    } catch (err: any) {
      sweetMixinErrorAlert(err?.message ?? t('Error')).then();
    }
  };
  ...
  return (
    <Box component={'div'} className={'content cs-admin-page'}>
      <Box className={'title flex_space'}>
        <Typography variant={'h2'}>{t('FAQ Management')}</Typography>
      ...
      <Box className={'table-wrap'}>
        ...
        <Stack gap={1.2} sx={{ p: '14px' }}>
          {faqList.map((faq) => (
            <Stack key={faq._id} sx={{ border: '1px solid #ececec', borderRadius: '10px', p: '14px', background: '#fff' }}>
              <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                <Typography variant={'h6'}>{faq.noticeTitle}</Typography>
                <Chip
                  size={'small'}
                  label={faq.noticeStatus}
                  color={faq.noticeStatus === NoticeStatus.ACTIVE ? 'success' : 'default'}
                />
              </Stack>
              <Divider sx={{ my: '8px' }} />
              <Typography sx={{ mb: '8px', color: '#424242' }}>{faq.noticeContent}</Typography>
              <Stack direction={'row'} gap={1}>
                <Button size={'small'} variant={'outlined'} onClick={() => openEditHandler(faq)}>
                  {t('Edit')}
                </Button>
                <Button size={'small'} variant={'outlined'} onClick={() => toggleStatusHandler(faq)}>
                  {faq.noticeStatus === NoticeStatus.ACTIVE ? t('Set HOLD') : t('Set ACTIVE')}
                </Button>
                <Button size={'small'} color={'error'} variant={'outlined'} onClick={() => removeHandler(faq._id)}>
                  {t('Delete')}
                </Button>
              </Stack>
            </Stack>
