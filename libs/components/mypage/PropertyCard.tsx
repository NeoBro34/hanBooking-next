			...
			<Stack className="property-card-box">
				...
				<Stack className="information-box" onClick={() => pushPropertyDetail(property?._id)}>
					...
					<Typography className="price">
						<strong>${formatterStr(property?.propertyPricePerNight)}</strong>
					</Typography>
				</Stack>
				...
				<Stack className="status-box">
					<Stack className={property.propertyStatus === 'ACTIVE' ? 'status-active' : 'status-paused'} onClick={handleClick}>
				...
				{!memberPage && (
					...
					>
						...
						{property.propertyStatus === 'PAUSED' && (
							<>
								<MenuItem
									disableRipple
									onClick={() => {
				...
				<Stack className="views-box">
					<Typography className="views"><RemoveRedEyeIcon sx={{mr:"5px"}}/>{property.propertyViews.toLocaleString()}</Typography>
				</Stack>
				{!memberPage && (
